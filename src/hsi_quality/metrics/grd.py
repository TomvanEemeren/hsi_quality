...
from .metric import NoReferenceMetric

from hypso import Hypso2

class GRD(NoReferenceMetric):
    ...
    def calculate(self, satobj: Hypso2, metadata: pd.Series):
        ...
        for band in range(cube.shape[2]):
            ...
            edge_response, x0_interp = self.fit_edge_response(intensities)

            fwhm = self.calculate_fwhm(edge_response, x0_interp)
    ...
    def calculate_fwhm(self, edge_response, x0_interp):
    ...
    def fit_edge_response(self, intensities):
        N = 1000
        x0 = np.arange(len(intensities))
        x0_interp = np.linspace(0, len(intensities)-1, N)
        intensities_interp = si.griddata(x0, intensities, x0_interp, method="cubic")
        intensities_interp_linear = si.griddata(x0, intensities, x0_interp, method="linear")

        d = np.min(intensities)
        b = self.length // 2
        c = -0.5
        a = 2*(intensities_interp[N//2] - d)

        fit = so.curve_fit(self.edge_function, x0_interp, intensities_interp_linear, p0=[a, b, c, d])

        edge_response = self.edge_function(x0_interp, fit[0][0], fit[0][1], fit[0][2], fit[0][3])

        return edge_response, x0_interp
